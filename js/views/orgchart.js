...
const OrgChartView = {
  ...
  // SmartHR役職は「レベル区分(一般社員/管理職)＋具体役職(部長/課長…)」の組合せ。
  // 具体役職が無く区分だけの人もいるため、末尾に 管理職→一般社員 を置き、
  // 「管理職が一般社員より下」になる不具合を防ぐ（具体役職があればそちらが優先される）。
  POSITION_RANK: ['会長', '社長', '代表取締役', '専務', '常務', '社外取締役', '監査等委員', '本部長', '上席執行役員', '執行役員', '部長', '副部長', '室長', '所長', '副所長', '課長', '作業所長', '管理職', '一般社員'],
  ...
  // 各役職文字列ごとに「最も具体的（最長一致）」な役職を採用する。
  // 「副部長」は「部長」も部分一致するため、長い方を優先しないと部長と同順位になり、
  // タイブレーク（社員番号順）次第で副部長が部長より上に出てしまう不具合の対策。
  // 同様に 本部長/副所長/作業所長 なども正しく区別される。
  positionRank(positions) {
    let best = 999;
    (positions || []).forEach(p => {
      const s = String(p || '');
      let idx = -1, len = -1;
      this.POSITION_RANK.forEach((t, i) => {
        if (s.includes(t) && t.length > len) { len = t.length; idx = i; }
      });
