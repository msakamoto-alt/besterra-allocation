...
const ProspectsView = {
  ...
  // 一般語と数字（年号・番号など）を除いた「核」（固有名詞部分）を返す。
  // 数字を残すと「2026」のような年号だけで一致して誤発火するため核からは除外する。
  // ※ 完全一致/内包の厳密判定(①)は normName（数字あり）で別途行うので号機番号等は失われない。
  coreName(s) {
    return this.normName(s).replace(this.CORE_STOP, '').replace(/[0-9]+/g, '');
  },
  ...
  // 4だと「センター」「コークス」等の4文字設備名で無駄発火するため5に設定。
  SIM_MIN_LCS: 5,
