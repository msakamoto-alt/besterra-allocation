...
- 稼働定義（2026-07-10確定・07-13準備期間追加・変更時はSQLコメントも同期）: その月に1日でも配置バーが
  ...
  **または** 専従・派遣・配置キープ（work_mode。期間設定があればその期間の月のみ）。
  ※「配置キープ」（2026-08-17追加）= SF・見込み案件に未登録の予定案件のための枠確保。WORK_MODES（sync/db.js）に
  1エントリ足すだけでプルダウン・色帯・凡例・空き帯抑制・週次稼働が全て追従する設計。
